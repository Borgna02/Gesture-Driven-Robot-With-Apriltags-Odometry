...
class ImageUtils:
    ...
    def capture_image(device: cv2.VideoCapture, gesture_controller: GestureController, is_from_phone: bool):
        # Svuota il buffer della camera leggendo tutti i frame accumulati
        for _ in range(3):  # Legge 3 frame dal buffer per ottenere l'ultimo disponibile
            grabbed = device.grab()
            if not grabbed:
                break

        # Legge solo l'ultimo frame disponibile
        ret, image = device.retrieve()
        
        if not ret:
            return

        # Ottimizza le operazioni di trasformazione
        if is_from_phone:
            image = cv2.rotate(image, cv2.ROTATE_90_CLOCKWISE)
        
        # Applica trasformazioni in-place per ridurre l'uso di memoria
        cv2.flip(image, 1, dst=image)  # Riflesso orizzontale direttamente sull'immagine originale

        # Rilevamento mani con ottimizzazioni
        gesture_controller._tracker.handsFinder(image)
        
        # Calcola dimensioni una volta sola
        height, width = image.shape[:2]
        new_height = int(Constants.WINDOW_WIDTH * height / width)
        
        # Ridimensionamento ottimizzato con interpolazione bilineare
        image = cv2.resize(image, (Constants.WINDOW_WIDTH, new_height), 
                        interpolation=cv2.INTER_LINEAR)

        # Elaborazione landmarks
    ...
    def send_image(image):
        address = os.getenv('VIDEO_OUTPUT_ADDRESS')
        if not address:
            return

        success, encoded_image = cv2.imencode('.jpg', image, [int(cv2.IMWRITE_JPEG_QUALITY), 70])
        ...
        try:
            ...
            print(f"Errore invio: {str(e)[:50]}")  # Log ridotto
...
# Se non è impostato l'url, allora uso la webcam del computer
WEBCAM_URL = os.getenv("DROIDCAM_URL", 0)
print("WEBCAM_URL: ", WEBCAM_URL)
...
def setup():
    ...
    device = cv2.VideoCapture(WEBCAM_URL)
    device.set(cv2.CAP_PROP_BUFFERSIZE, 1)
    device.set(cv2.CAP_PROP_FPS, 60)
    ...
    is_from_phone = False if WEBCAM_URL == 0 else True
...
def loop():
    ...
    if serviceAuto and gestureManualChan and gestureModeChan:
        if (chronoImage.stop() > 0.05):
            ...
            try:
                ImageUtils.send_image(controller._image)
            except Exception as e:
